...
export default function OutboundWaveDetails() {
  ...
  const addOrdersMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t('outbound.ordersAdded', 'ההזמנות נוספו לגל בהצלחה'));
  ...
  const orderColumns: ColumnDef<OutboundOrder>[] = [
    ...
    { accessorKey: 'customer.name', header: t('outbound.customer', 'לקוח') },
    { accessorKey: 'status', header: t('outbound.status', 'סטטוס'), cell: ({ row }) => {
    ...
    { header: t('outbound.lines', 'שורות'), cell: ({ row }) => row.original.lines?.length || 0 },
  ];

  const taskColumns: ColumnDef<PickTask>[] = [
    { accessorKey: 'task_number', header: 'משימה', cell: ({ row }) => <span className="font-mono text-xs">{row.original.task_number}</span> },
    { accessorKey: 'from_location.name', header: 'מיקום', cell: ({ row }) => <div className="flex items-center gap-1"><MapPin className="w-3 h-3 text-muted-foreground" />{row.original.from_location?.name}</div> },
    { accessorKey: 'product.sku', header: 'מק"ט', cell: ({ row }) => row.original.product?.sku },
    { accessorKey: 'product.name', header: 'שם מוצר', cell: ({ row }) => row.original.product?.name },
    { accessorKey: 'quantity', header: 'כמות לליקוט', cell: ({ row }) => <span className="font-bold">{row.original.quantity}</span> },
    ...
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <Dialog open={isAddOrdersOpen} onOpenChange={setIsAddOrdersOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>הוספת הזמנות לגל</DialogTitle>
            <DialogDescription>
              בחר הזמנות מאושרות (Verified) מהרשימה להוספה לגל הנוכחי.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="py-4 space-y-4">
            <div className="relative">
              ...
              <Input
                placeholder="חפש לפי מספר הזמנה או לקוח..."
                className="pr-9"
                value={searchTerm}
            ...
            <div className="border rounded-md">
              <ScrollArea className="h-[300px]">
                {isOrdersLoading ? (
                  <div className="p-8 text-center text-muted-foreground">טוען הזמנות...</div>
                ) : filteredAvailableOrders.length === 0 ? (
                  <div className="p-8 text-center text-muted-foreground">לא נמצאו הזמנות זמינות להוספה.</div>
                ) : (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">{order.customer?.name} | {order.lines?.length} שורות</div>
                          </div>
                        </div>
            ...
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              <span>נבחרו {selectedOrdersToAdd.length} הזמנות</span>
            </div>
          </div>

          <DialogFooter>
            <Button variant="outline" onClick={() => setIsAddOrdersOpen(false)}>ביטול</Button>
            <Button 
              onClick={() => addOrdersMutation.mutate(selectedOrdersToAdd)} 
              disabled={selectedOrdersToAdd.length === 0 || addOrdersMutation.isPending}
            >
              {addOrdersMutation.isPending ? "מוסיף..." : "הוסף לגל"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
