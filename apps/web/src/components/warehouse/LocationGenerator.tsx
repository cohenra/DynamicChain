...
export function LocationGenerator({ warehouseId, zones, onSuccess, onCancel }: LocationGeneratorProps) {
  ...
  useState(() => {
    ...
    if (bayStart > 0 && bayEnd >= bayStart && levelStart > 0 && levelEnd >= levelStart) {
      const count = (bayEnd - bayStart + 1) * (levelEnd - levelStart + 1);
  ...
  const getPreviewString = () => {
    const { zone_id, aisle, bay_start, bay_end, level_start, level_end } = watchValues;

    if (!zone_id || !aisle || !bay_start || !bay_end || !level_start || !level_end) {
    ...
    if (bayStartNum <= 0 || bayEndNum < bayStartNum || levelStartNum <= 0 || levelEndNum < levelStartNum) {
      return t('locations.previewInvalid');
    }

    const firstLocation = `${aisle}-${String(bayStartNum).padStart(2, '0')}-${String(levelStartNum).padStart(2, '0')}`;
    const lastLocation = `${aisle}-${String(bayEndNum).padStart(2, '0')}-${String(levelEndNum).padStart(2, '0')}`;
